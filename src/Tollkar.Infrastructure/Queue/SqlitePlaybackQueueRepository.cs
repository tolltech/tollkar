...
internal sealed class SqlitePlaybackQueueRepository(string databasePath) : IPlaybackQueueRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = "SELECT q.Id,q.SongId,s.Title,s.Artist,s.Capabilities,q.Position,q.UserId,f.ProviderId FROM PlaybackQueue q JOIN Songs s ON s.Id=q.SongId JOIN Files f ON f.SongId=s.Id WHERE q.UserId=$user ORDER BY q.Position;";
